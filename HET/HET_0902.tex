...
$\nu_{max}$ layouts. The GAS layouts include a small solar component,
decreasing with increasing K values. The GAS layouts are noticeably
...
unlike the $\beta$ and $\nu_{max}$ layout definitions, the GAS routine has
...
%The $\beta$ layouts are shown in \cref{fig:betaOpt} and the $\nu_{max}$ layouts in \cref{fig:cfMaxOpt}. In
...
$\alpha$ values are 1. Even for the GAS layouts, only a minor solar
