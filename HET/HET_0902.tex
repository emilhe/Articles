\documentclass[a4paper, 12pt, sort&compress]{elsarticle}%5p for double column
% 
\usepackage{ucs}
...
%Optimal Portfolio Theory to explore heterogeneous distributions of
...
European countries are discussed. \Cref{sec:sensitivity-analysis}
...
flow\cite{Magnus}. %\footnote{This is only strictly true for unconstained
...
% TODO: ADD COMMENTS!!
% The CS layouts are very similar to the $\nu_{max}$ layouts, but a
% small solar component is introduced by shifting countries with a low
% $\nu^{W}$ from wind to solar PV.

%The general tendency for the optimized layouts is that wind capacities
%are moved towards the North Sea countries (Denmark, Netherlands, Great
%Britain, Ireland, Norway, etc.). It seems like a reasonable choice as
%their wind resource are generally better than for the central and
%southern countries. 
...
\begin{figure}[h!]
  ...
  \includegraphics[width = 0.8\columnwidth]{/VE50cuckooK=2@TRANS10kLINKS}
...
% problem in finance, where a portfolio of investments must be chosen
...
% representation of the portfolio of wind capacities in Europe. Each
...
% without a large decrease in the risk of the investment. Portfolio
% theory allows us to define a Pareto front of optimal portfolios,
...
% portfolio theory. We choose one of these interpolations as a
% \textbf{ Pareto-optimal layout}. The lines connecting these points
% run from $\alpha = 0$ in the solar-only portfolio (bottom right) to
% $\alpha = 1$ in the wind-only portfolio (top right). It is clear that
